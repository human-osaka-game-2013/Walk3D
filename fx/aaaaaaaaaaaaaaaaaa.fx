...
// 頂点シェーダ
VS_OUTPUT_S ZBufferCalc_VS(float4 Pos : POSITION, float4 W : BLENDWEIGHT, float3 Norm : NORMAL, float2 inTexCoord : TEXCOORD0 ,int4 idx     : BLENDINDICES)
{
...
//-----------------------------------------
   ...
   float4x4 matCombWorld = 0.f;           // 合成ワールド変換行列
   //int num = iBlendNum - 1;
   // ワールド変換行列をブレンド
//for ( int i = 0; i < 3; i++ )
...
//if( num > 4 ) num = 4;
 //  if( blendnum >= 2)LastBlendWeight += Weight[0];   // 最後の重みをここで計算しておく
  // if( blendnum >= 2)matCombWorld += matWorld[0] * Weight[0];

  // if( blendnum >= 3)LastBlendWeight += Weight[1];   // 最後の重みをここで計算しておく
  // if( blendnum >= 3)matCombWorld += matWorld[1] * Weight[1];

   for(int i=0; i<iBlendNum-1; i++)
   {
      LastBlendWeight += Weight[0];   // 最後の重みをここで計算しておく
      matCombWorld += matWorld[0] * Weight[0];
   }
   //matWorld[2] = matWorld[0];
  // float4x4 mattemp = matWorld[2];
  // if( blendnum >= 4)LastBlendWeight += Weight[2];   // 最後の重みをここで計算しておく
  // if( blendnum >= 4)matCombWorld += mattemp * Weight[2];

   // 最後の重みを足し算
  // matCombWorld += matWorld[0] * (1.0f-LastBlendWeight);
