...
struct VS_OUTPUT_S
{
  ...
  float4 DefMapTex : TEXCOORD1;      // 貼り付けるテクスチャ
  float2 Col : COLOR;				// ディフューズ色
};
...
// ピクセルシェーダ
float4 ZBufferPlot_PS( float4 Col : COLOR, float4 ZCalcTex : TEXCOORD1, float2 texCoord : TEXCOORD0 ) : COLOR
{
 //return ZCalcTex.z / ZCalcTex.w;
  float4 Color;
